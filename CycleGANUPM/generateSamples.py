...
from StyleGANUPM.Components import make_dataset
...
gen_AB = CycleGenerator.Generator(3, 3).to(device)
gen_BA = CycleGenerator.Generator(3, 3).to(device)
gen_opt = torch.optim.RMSprop(list(gen_AB.parameters()) + list(gen_BA.parameters()), lr=lr)
disc_A = CycleDiscriminator.Discriminator(3).to(device)
disc_A_opt = torch.optim.RMSprop(disc_A.parameters(), lr=lr)
disc_B = CycleDiscriminator.Discriminator(3).to(device)
disc_B_opt = torch.optim.RMSprop(disc_B.parameters(),lr=lr ) ##Adam(disc_B.parameters(), lr=lr, betas=(0.5, 0.999))
...
training_dir = 'CycleTraining'
load_dir = 'models'
gen_disc_load = 'cycleGAN_398.pth'
